...
2. $A$ y $B$ son **tipos juiciosamente iguales** en un contexto $1\Gamma$. Expresamos 
  este juicio como $$\Gamma \vdash A \stackrel{\cdot}{=} B ~~~ \text{type}$$
3. $a$ es un **elemento** de tipo $A$ en un contexto $1\Gamma$. Expresamos este juicio
  como $$ \Gamma \vdash a: A. $$
4. $a$ y $b$ son **elementos juiciosamente iguales** de tipo $1A$ en un contexto 
...
de tipos sobre $A$ en el contexto $\Gamma$ es un tipo $1B(x)$ en el contexto 
$\Gamma, x: A$. En otras palabras, en la situación en la que 
$\Gamma, x: A \vdash B(x) ~~~ \text{type}$, decimos que $B$ es una familia de tipos 
sobre $A$ en el contexto $\Gamma$. Alternativamente, decimos que $1B(x)$ es un tipo
...
el tipo $a = x$ en el contexto $\Gamma, x: A$. El tipo $1a = x$ en el contexto 
$\Gamma, x: A$ es un ejemplo de una familia de tipos sobre $A$ en el contexto 
$\Gamma$. 

<div id="def-1-2-2">
<Aside type="tip" title="Definición 1.2.2">
Consideremos una familia de tipos $B$ sobre $A$ en un contexto $1\Gamma$. Una **sección** de la familia \(B\) sobre \(A\) en el contexto \(\Gamma\) es un elemento de tipo \(B(x)\) en el contexto \(\Gamma, x: A\), es decir, en el juicio 
$$
  \Gamma, x: A \vdash b(x): B(x).
$$
Decimos que $b$ es una sección de la familia $B$ sobre $1A$ en el contexto \(\Gamma\).
...
Similarmente, podemos substituir $a$ por $x$ en un elemento $1c: C$ para obtener un 
...
$\Gamma$ y si tenemos $a: A$, entonces también decimos que $1B[a/x]$ es la **fibra**
de $B$ en $a$ y escribimos $1B(a)$ para referirnos a esta fibra.
Cuando $b$ es una sección de la familia $B$ sobre $1A$ en el contexto \(\Gamma\), 
llamamos al elemento $b[a/x]$ el **valor** de $b$ en $1a$ y escribimos \(b(a)\).
</Aside>
</div>
...
En la situación más simple, tenemos dos tipos $A$ y $B$ en un contexto $1\Gamma$.
...
$$
El tipo $B$ en el contexto $\Gamma, x: A$ es llamado la **familia constante** $1B$ o 
la **familia trivial** $B$.

### Los elementos genéricos
Si tenemos un tipo $A$ en un contexto $\Gamma$, podemos debilitar a $1A$ por sí mismo 
...
$\Gamma, x: A$ es también un elemento de tipo $A$ en el contexto $1\Gamma, x: A$.
